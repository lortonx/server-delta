...
Parse.Cloud.beforeSave('Test', () => {
    throw new Parse.Error(119, 'Saving test objects is not available.');
});
...
Parse.Cloud.afterSave('_User', async (data) => {
    ...
    for (const KEY in ACL_TARGET.permissionsById) {
        if (
            ACL_TARGET.getReadAccess(KEY) != ACL_USER.getReadAccess(KEY) ||
            ACL_TARGET.getWriteAccess(KEY) != ACL_USER.getWriteAccess(KEY)
        ) {
...
//     class DbStats extends Parse.Object {
//         className = 'DbStats';
//     }
