...
export default class BmcEvent extends Parse.Object {
    ...
    async findUserByEmail(email) {
        return await new Parse.Query(Parse.User).equalTo('email', email).first({ useMasterKey: true });
    }
    async findMyExtra() {
        return await BmcExtra.findRecord(this.get('supporter_email'), null, this.get('amount'), this.get('created_at'));
    }
    async findMySupport() {
